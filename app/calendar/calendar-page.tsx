...
type Member = { email: string; name: string; status: 'active' | 'invited' };
...
export default function CalendarPage({
  ...
  useEffect(() => {
    void Promise.all([fetch('/api/tasks'), fetch('/api/members')]).then(
      async ([taskResponse, memberResponse]) => {
        if (taskResponse.ok && memberResponse.ok) {
          setTasks(await taskResponse.json());
          setMembers(await memberResponse.json());
        } else setNotice('No pudimos cargar el calendario.');
      },
    );
  }, []);
  ...
  async function changeStatus(task: Task, nextStatus: Status) {
    ...
    if (response.ok) {
      ...
      setSelected(updated);
    } else setNotice('No tienes permiso para actualizar esta tarea.');
  }
  ...
  const memberName = (email: string) =>
    members.find((member) => member.email === email)?.name ?? email;
  const canUpdate =
    selected &&
    (user.role === 'owner' ||
      selected.assignee.toLowerCase() === user.email.toLowerCase());
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="flex items-center gap-2 text-sm font-medium text-primary">
              <CalendarDays className="size-4" /> Planificación
            </p>
            <h1 className="mt-1 text-3xl font-bold">Calendario</h1>
            <p className="mt-2 text-sm text-muted-foreground">
              Fechas límite de {workspace.name}.
            </p>
          </div>
        ...
        <div className="mt-4 overflow-hidden rounded-2xl border bg-card">
          ...
          <div className="grid grid-cols-7">
            {cells.map((date) => {
              ...
              const dayTasks = filtered.filter((task) => task.dueDate === iso);
              const currentMonth = date.getMonth() === month.getMonth();
              return (
                <div
                  key={iso}
                  className={`min-h-28 border-b border-r p-1.5 sm:min-h-32 sm:p-2 ${currentMonth ? 'bg-card' : 'bg-muted/25'}`}
                >
                  <div
                    className={`mb-1 grid size-7 place-items-center rounded-full text-xs ${iso === today ? 'bg-primary font-bold text-primary-foreground' : currentMonth ? '' : 'text-muted-foreground'}`}
                  >
                    {date.getDate()}
                  </div>
                  <div className="space-y-1">
                    {dayTasks.slice(0, 3).map((task) => (
                      <button
                        ...
                        className={`block w-full truncate rounded-md px-1.5 py-1 text-left text-[10px] font-medium sm:text-xs ${task.status === 'done' ? 'bg-[#e7f7f1] text-[#238363]' : task.priority === 'high' ? 'bg-[#ffeded] text-destructive' : 'bg-[#f0edff] text-[#5b48d6]'}`}
                      >
                        {task.title}
                      </button>
                    ))}
              ...
              <div className="mt-4 space-y-3 text-sm">
                <div className="flex items-center justify-between">
                  ...
                  <span>
                    {new Intl.DateTimeFormat('es-CO', {
                      dateStyle: 'medium',
                    }).format(new Date(`${selected.dueDate}T12:00:00`))}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1 text-muted-foreground">
                    <CircleUserRound className="size-4" /> Responsable
                  </span>
                  <span>{memberName(selected.assignee)}</span>
                </div>
                <div className="flex items-center justify-between">
