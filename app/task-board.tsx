...
export default function TaskBoard({
  ...
  const [form, setForm] =
    useState<Omit<Task, 'id'>>(blankTask);
  ...
  async function loadTasks() {
    try {
      ...
      const loadedMembers =
        (await memberResponse.json()) as Member[];

      setMembers(loadedMembers);

      if (
        loadedMembers.length &&
        blankTask.assignee === 'CM'
      ) {
        blankTask.assignee =
          loadedMembers[0].email;
      }
    } catch {
      setNotice(
        'No pudimos cargar tus tareas. Intenta de nuevo.',
      );
    } finally {
      setLoading(false);
    }
  ...
  async function moveTaskTo(
    ...
    if (!canMoveTask(task)) {
      setNotice(
        'Solo puedes avanzar las tareas que tienes asignadas.',
      );
                  ...
                  >
                    <div className="flex items-center justify-between px-1 pb-3">
                      <div className="flex items-center gap-2">
                        <span
                          className={`size-2 rounded-full ${column.tone}`}
                        />
        ...
        <DialogContent className="sm:max-w-md">
          ...
          >
            <DialogHeader>
              ...
              <DialogDescription>
                Recibirá un enlace seguro
                para entrar, ver el tablero
                y recibir tareas.
              </DialogDescription>
            </DialogHeader>
