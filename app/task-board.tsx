...
type Member = { email: string; name: string; role: 'owner' | 'member'; status: 'active' | 'invited' };
...
export default function TaskBoard({ user }: { user: { name: string; email: string } }) {
  ...
  async function loadTasks() {
    try {
      const [taskResponse, memberResponse] = await Promise.all([fetch('/api/tasks'), fetch('/api/members')]);
      ...
      if (loadedMembers.length && blankTask.assignee === 'CM') blankTask.assignee = loadedMembers[0].email;
  ...
  async function inviteMember(event: React.FormEvent) {
    ...
    const response = await fetch('/api/members', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ email: inviteEmail, name: inviteName }) });
    if (!response.ok) { setNotice('Revisa el email e intenta de nuevo.'); return; }
    const member = await response.json() as Member;
    setMembers((current) => [...current.filter((item) => item.email !== member.email), member]);
    setInviteOpen(false); setInviteEmail(''); setInviteName(''); setNotice('Invitación preparada'); setTimeout(() => setNotice(''), 2500);
  }
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-60 border-r bg-sidebar px-4 py-5 lg:flex lg:flex-col">
        ...
        <div className="mt-auto"><a className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-muted-foreground" href="/signout-with-chatgpt?return_to=/"><Settings2 className="size-4"/>Cerrar sesión</a><div className="mt-3 flex items-center gap-3 border-t pt-4"><div className="grid size-9 place-items-center rounded-full bg-[#efeaff] text-xs font-bold text-[#5b48d6]">{initials(user.name)}</div><div className="min-w-0"><p className="truncate text-sm font-medium">{user.name}</p><p className="truncate text-xs text-muted-foreground">{user.email}</p></div></div></div>
      </aside>
      ...
      <section className="lg:pl-60">
        ...
        <div className="px-4 py-6 sm:px-7 sm:py-8">
          <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="mb-1 text-sm font-medium text-primary">Viernes, 28 de agosto</p><h1 className="text-2xl font-bold tracking-tight sm:text-3xl">Buenos días, {user.name.split(' ')[0]}</h1><p className="mt-1 text-sm text-muted-foreground">Estas son las actividades que necesitan tu atención.</p></div><div className="flex items-center gap-2 rounded-full border bg-card px-3 py-2 text-xs text-muted-foreground"><CheckCircle2 className="size-4 text-[#35b78a]"/><strong className="text-foreground">{tasks.filter((task) => task.status === 'done').length}</strong> completadas</div></div>
          <div className="mt-8 flex items-center justify-between"><div><h2 className="text-lg font-semibold">Mi tablero</h2><p className="text-xs text-muted-foreground">{tasks.length} actividades · {members.length} {members.length === 1 ? 'persona' : 'personas'}</p></div><Button onClick={() => setInviteOpen(true)} variant="outline" className="rounded-xl"><Users/>Invitar</Button></div>
      ...
      <Dialog open={inviteOpen} onOpenChange={setInviteOpen}>
        <DialogContent className="sm:max-w-md"><form onSubmit={inviteMember}><DialogHeader><DialogTitle>Invitar participante</DialogTitle><DialogDescription>Podrá ver el tablero y recibir tareas asignadas.</DialogDescription></DialogHeader><div className="mt-5 space-y-4"><label className="block text-xs font-semibold">Nombre<Input value={inviteName} onChange={(e) => setInviteName(e.target.value)} className="mt-1 h-10" placeholder="Nombre del participante"/></label><label className="block text-xs font-semibold">Email<Input type="email" required value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} className="mt-1 h-10" placeholder="persona@ejemplo.com"/></label>{members.length > 0 && <div className="rounded-xl bg-muted p-3"><p className="text-xs font-semibold">Personas en el espacio</p><div className="mt-2 space-y-2">{members.map((member) => <div key={member.email} className="flex items-center gap-2 text-xs"><span className="grid size-7 place-items-center rounded-full bg-[#e9e4ff] font-bold text-[#5b48d6]">{initials(member.name)}</span><span className="min-w-0 flex-1 truncate">{member.name}</span><Badge variant="outline">{member.status === 'active' ? 'Activo' : 'Invitado'}</Badge></div>)}</div></div>}</div><DialogFooter className="mt-5"><DialogClose render={<Button type="button" variant="outline"/>}>Cancelar</DialogClose><Button type="submit">Preparar invitación</Button></DialogFooter></form></DialogContent>
      </Dialog>
    </main>
  );
}
